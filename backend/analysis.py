...
def _candidates_numpy(
    ...
    # Build upper-triangle index pairs in chunks to cap peak memory.
    CHUNK = 500
    ii_out, jj_out, dist_out = [], [], []

    for start in range(0, n, CHUNK):
        end = min(start + CHUNK, n)
        # rows: anchors[start:end], cols: anchors[start+1:] (avoid duplicates)
        row_lats = lats[start:end, np.newaxis]   # (chunk, 1)
        row_lons = lons[start:end, np.newaxis]
        for col_start in range(0, n, CHUNK):
            col_end = min(col_start + CHUNK, n)
            col_lats = lats[np.newaxis, col_start:col_end]  # (1, chunk)
            ...
            # Only upper triangle: global j > global i
            ...
            # Vectorized haversine for these candidates
